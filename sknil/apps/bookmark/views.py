...
def category_add(request):
    ...
    if categories >= 50 and request.user.userprofile.plan == 'pro':
        canAdd = 'You cannot have more than 50 categories when you are on a pro plan!'

    if categories >= 5 and request.user.userprofile.plan == 'basic':
        canAdd = 'You cannot have more than 5 categories when you are on a basic plan!'
...
def bookmark_add(request, category_id):
    ...
    if bookmarks >= 50 and request.user.userprofile.plan == 'basic':
        canAdd = 'You cannot have more than 50 bookmarks when you are on a basic plan!'
...
def bookmark_edit(request, category_id, bookmark_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'The changes have been applied')
